...
namespace TgTranslator.Services.Handlers
{
    public class CallbackQueryHandler : ICallbackQueryHandler
    {
        ...
        private ImmutableDictionary<string, bool> _supportedCommands;
        
        public CallbackQueryHandler(BotMenu botMenu)
        {
            _botMenu = botMenu;
        ...
        public async Task HandleCallbackQueryAsync(CallbackQuery callbackQuery)
        {
            ...
            if (!_supportedCommands.Contains(new KeyValuePair<string, bool>(command, arguments != null)))
                throw new UnsupportedCommand(callbackQuery.Data);
            
            switch (command)
            {    
                case "switch":
                        await _botMenu.SwitchMenu(
